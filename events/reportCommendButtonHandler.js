...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!(action.startsWith("praise") || action === "warn" || action === "punish" || action === "noaction" || action === "disapprove" || action === "falsecommend" || action === "falsereport")) {
            return;
        }
        ...
        try {
            ...
            // Handle praise, warn, punish, and noaction
            if (action.startsWith("praise") || action === "warn" || action === "punish" || action === "disapprove" || action === "noaction") {
                if (action === "praise1") {
                    adjustment = 1;
                    ...
                    actionTaken = "warned for the commendation and receives -5 social credit score. This shouldn't be commended";
                    adjustment = -5;
                }

                // Apply the adjustment to the target user (reported or commended user) if needed
                if (adjustment !== 0) {
                    const incrementField = adjustment > 0 ? "stats.socialCreditGained" : "stats.socialCreditLost";
                    await profileModel.findOneAndUpdate(
                        { userId: targetUser },
                        {
                            $inc: {
                                socialCreditScore: adjustment,
                                [incrementField]: Math.abs(adjustment),
                            },
                        }
                    );
                }
                ...
                if (action !== "noaction") {
                    await profileModel.findOneAndUpdate(
                        { userId: sourceUserId },
                        {
                            $inc: {
                                socialCreditScore: 2,
                                "stats.socialCreditGained": 2,
                                [action.startsWith("praise") ? "stats.commendsMade" : "stats.reportsMade"]: 1,
                            },
                        }
                    );
                }
            ...
            if (action === "falsecommend" || action === "falsereport") {
                ...
                await profileModel.findOneAndUpdate(
                    { userId: targetUser },
                    {
                        $inc: {
                            socialCreditScore: adjustment,
                            "stats.socialCreditLost": Math.abs(adjustment),
                            [action === "falsereport" ? "stats.falseReports" : "stats.falseCommends"]: 1,
                        },
                    }
                );
                ...
                const outcomeEmbed = new EmbedBuilder()
                    ...
                    .setColor(0xff0000)
                    .setDescription(
                        `<@${sourceUserId}> has been penalized for a false ${actionTaken}. Their current social credit score is ${updatedSourceProfile.socialCreditScore}.`
                    )
                    .setTimestamp();
